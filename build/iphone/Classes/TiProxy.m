...
//Rare exceptions to indicate a bug in the _frogcounter code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_frogcounter = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_frogcounter]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.frogcounter.module"
